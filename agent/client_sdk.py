...
work to the network instead of running it locally.
    ...
    result = cc.run_tensor_op("matmul", a, b, account_name="marvin")
"""
...
def _submit(coordinator, account_name, task_type, payload: dict, cpu_limit, ram_limit_mb, timeout_s):
...
def run_tensor_op(op: str, a, b=None, *, account_name: str, coordinator="http://127.0.0.1:8000",
    ...
    job_id = _submit(coordinator, account_name, "tensor_op", payload,
...
def run_llm_infer(prompt: str, *, model_name: str, account_name: str,
    ...
    is a provider-side asset, never sent as part of the job.
    """
    payload = {"prompt": prompt, "max_tokens": max_tokens, "temperature": temperature}
    if system:
        payload["system"] = system
    job_id = _submit(coordinator, account_name, f"llm_infer:{model_name}", payload,
                      cpu_limit, ram_limit_mb, timeout_s)
    result = _wait_for_result(coordinator, job_id, max_wait_s=max_wait_s)
    return result["text"]


def run_onnx_infer(model_bytes: bytes, input_array, *, account_name: str,
                    coordinator="http://127.0.0.1:8000", input_name=None,
                    cpu_limit=1.0, ram_limit_mb=1024, timeout_s=60, max_wait_s=300) -> np.ndarray:
    """Offload inference of an ONNX model on one input tensor."""
    payload = {
    ...
    job_id = _submit(coordinator, account_name, "onnx_infer", payload,
                      cpu_limit, ram_limit_mb, timeout_s)
    result = _wait_for_result(coordinator, job_id, max_wait_s=max_wait_s)
